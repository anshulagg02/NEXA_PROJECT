...
if(isset($_POST['regs']))
{
    ...
    if(empty($fname)|| empty($lname)|| empty($email)|| empty($lic)|| empty($ph)|| empty($pass) || empty($gender))
    {
        ...
        if($pass==$cpass){
        $sql2="SELECT *from TEST_DRIVE where EMAIL='$email'";
        $res=mysqli_query($con,$sql2);
        if(mysqli_num_rows($res)>0){
            echo '<script>alert("TEST DRIVE ALREADY REQUESTED")</script>';
            echo '<script> window.location.href = "index.php";</script>';

        }
        else{
        $sql="insert into TEST_DRIVE (FNAME,LNAME,EMAIL,LIC_NUM,PHONE_NUMBER,PASSWORD,GENDER) values('$fname','$lname','$email','$lic',$ph,'$Pass','$gender')";
        $result = mysqli_query($con,$sql);
          
        if($result){
            echo '<script>alert("TEST DRIVE REQUESTED SUCCESFULLY OUR EXCECUTIVES WILL CONTACT YOU FOR AVAILABLE DATES AND TIMES")</script>';
            echo '<script> window.location.href = "index.php";</script>';       
           }
        else{
...
.invalid:before {
  ...
  content: "✖";
    ...
    <div>
      <h1 style="margin:auto;padding:0%;display: flex;justify-content: center;color:white;" id="fam" > JOIN OUR FAMILY OF CARS!</h1>
    </div>
      
    <div class="main">
        
        <div class="register">
        <h2>Register Here for Booking Test Drive</h2>
        ...
        <form id="register" action="register.php" method="POST">    
            ...
            id="psw" placeholder="Enter secure key" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" required>
            <br><br>
            <label>Confirm Secure Key: </label>
            <br>
            <input type="password" name="cpass" 
            id="cpsw" placeholder="Renter the secure key" required>
            <br><br>
            <tr>
    ...
    <div id="message">
  <h3>Secure Key must contain the following:</h3>
